...
*/
	...
	*/
	function FailTrans($message = null)
	{
		if ($this->debug) 
			if ($this->transOff == 0) {
				ADOConnection::outp("FailTrans outside StartTrans/CompleteTrans".(!empty($message) ? ': '.$message : ''));
			} else {
				ADOConnection::outp("FailTrans was called".(!empty($message) ? ': '.$message : ''));
				adodb_backtrace();
			}
		$this->_transOK = false;
	}
   ...
   */
	...
	*/
	...
	function _adodb_getdriver($provider,$drivername,$perf=false)
	{
		...
		case 'sqlite': $drivername = 'sqlite'; break;
