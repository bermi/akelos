...
class MemCachedClient {
    ...
    public function get($keys,$forcehost=false) {
        ...
        foreach ($keys as $k=>$key) {
            $socket = intval($this->_get_socket($key,$forcehost));
    ...
    public function set($key,$data,$exptime=0,$forcehost=false) {
    ...
    public function lock($key,$fail=true,$max_wait=30) {
    ...
    public function flush_all($host=false) {
    ...
    public function disconnect($host=false) {
        if ($host!==false) $hosts = array($host); else $hosts = $this->hosts;
    ...
    public function version($host=false) {
        if ($host!==false) $hosts = array($host); else $hosts = $this->hosts;
    ...
    public function incr($key,$value=1,$forcehost=false) {
    ...
    public function _incrdecr($method,$key,$value,$forcehost=false) {
        if (!$key) return $this->_error(ERR_NEED_KEY);
    ...
    public function _store($method,$key,$exptime,$data,$forcehost=false) {
    ...
    public function _socket_read($key,$len=10240,$forcehost=false,$attemptnumber=0) {
        $socket = $this->_get_socket($key,$forcehost);
        if (!$socket) return $this->_error(ERR_NO_SOCKET);
    ...
    public function _socket_readstr($key,$len=10240,$forcehost=false,$attemptnumber=0) {
        $socket = $this->_get_socket($key,$forcehost);
        if (!$socket) return $this->_error(ERR_NO_SOCKET);
    ...
    public function _get_active_sockets() {
        $res = array();
    ...
    public function _remove_socket($key,$forcehost=false) {
    ...
    public function _get_socket($key,$forcehost=false,$attempts=0) {
        //$host = $this->_get_host($key,$forcehost);
    ...
    public function _set_socket_recursive($host,$socket,&$sockets) {
    ...
    public function _set_socket($host,$socket) {
    ...
    public function _get_error($line) {
