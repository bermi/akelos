...
Active Support is the Akelos component responsible for providing PHP language extensions, utilities, and other transversal stuff. It offers a richer bottom-line at the language level, targeted both at the development of Akelos applications, and at the development of Akelos itself.

By referring to this guide you will learn the extensions to the PHP core classes and modules provided by Akelos.
...
The following values are considered to be blank in an Akelos application:
...
Here, the method Akelos runs to instantiate observers upon initialization has nothing to do if there are none:
...
Akelos has classes that act like +Date+ or +Time+ and follow this contract.

h4. +to_param+

All objects in Akelos respond to the method +to_param+, which is meant to return something that represents them as values in a query string, or as a URL fragments.
...
Several classes in Akelos overwrite this method.
...
Notably, the Akelos routing system calls +to_param+ on models to get a value for the +:id+ placeholder. +ActiveRecord#to_param+ returns the +id+ of a model, but you can redefine that method in your models. For example, given
...
also ignored. For example, Akelos controllers implement that method.
...
Akelos uses +alias_method_chain+ all over the code base. For example validations are added to +ActiveRecord#save+ by wrapping the method that way in a separate module specialised in validations.
...
WARNING. Active Support redefines +group_by+ in PHP 1.8.7 so that it still returns an ordered hash.
...
When the last argument in a method call is a hash, except perhaps for a +&block+ argument, PHP allows you to omit the brackets:

<php>
User.exists?(:email => $this->params['email'])
</php>

That syntactic sugar is used a lot in Akelos to avoid positional arguments where there would be too many, offering instead interfaces that emulate named parameters. In particular it is very idiomatic to use a trailing hash for options.
...
PHP has a builtin method +Hash#merge+ that merges two hashes:
...
PHP has builtin support for taking slices out of strings and arrays. Active Support extends slicing to hashes:
...
Akelos uses this method in a single place, also in the routing code. Multiline regexps are disallowed for route requirements and this flag eases enforcing that constraint.
...
For example, when an action of +PostsController+ is called Akelos tries optimistically to use +PostsHelper+. It is OK that the helper module does not exist, so if an exception for that constant name is raised it should be silenced. But it could be the case that +posts_helper.php+ raises a +NameError+ due to an actual unknown constant. That should be reraised. The method +missing_name?+ provides a way to distinguish both cases:
...
Akelos hijacks +LoadError.new+ to return a +MissingSourceFile+ exception:

<shell>
$ php -e 'require "nonexistent"'
...: no such file to load -- nonexistent (LoadError)
...
$ ./makelos runner 'require "nonexistent"'
...
For example, when an action of +PostsController+ is called Akelos tries to load +posts_helper.php+, but that file may not exist. That's fine, the helper module is not mandatory so Akelos silences a load error. But it could be the case that the helper module does exist, but it in turn requires another library that is missing. In that case Akelos must reraise the exception. The method +is_missing?+ provides a way to distinguish both cases:
