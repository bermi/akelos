...
public function hello_world(env)
...
By default, Akelos links to these assets on the current host in the public folder, but you can direct Akelos to link to assets from a dedicated assets server by setting +AkActionController.asset_host+ in your +config/environment.php+. For example, let's say your asset host is +assets.example.com+:

<php>
AkActionController.asset_host = "assets.example.com"
...
You can also cache multiple javascripts into one file, which requires less HTTP connections to download and can better be compressed by gzip (leading to faster transfers). Caching will only happen if +AkActionController.perform_caching+ is set to true (which is the case by default for the Akelos production environment, but not for the development environment).
...
<php>
my_array = {'first' => 1, 'second' => 'two', 'third' => [1,2,3]}
debug(my_array)
</php>
...
<php>
# Note: a @person variable will have been created in the controller (e.g. @person = new Person)
...
The params array has a nested person value, which can therefore be accessed with $this->params['person'] in the controller.
...
class Author extends ActiveRecord {
  has_many :posts
  public function name_with_initial() {
...
Accepts a container (array, array, enumerable, your type) and returns a string of option tags. 
...
Starts a form tag that points the action to an url configured with +url_for_options+ just like +AkActionController#url_for+.
...
<php>
page.$this->redirectTo(('controller' => 'accounts', 'action' => 'new')
</php>
...
For example, suppose you have a Posts controller with a show action. By default, calling this action will render +app/views/posts/show.html.tpl+. But if you set +Ak::lang() = :de+, then +app/views/posts/show.de.html.tpl+ will be rendered instead. If the localized template isn't present, the undecorated version will be used. This means you're not required to provide localized views for all cases, but they will be preferred and used if available. 

You can use the same technique to localize the rescue files in your public directory. For example, setting +Ak::lang() = :de+ and creating +public/500.de.html+ and +public/404.de.html+ would allow you to have localized rescue pages.

Since Akelos doesn't restrict the symbols that you use to set Ak::lang(), you can leverage this system to display different content depending on anything you like. For example, suppose you have some "expert" users that should see different pages from "normal" users. You could add the following to +app/controllers/application.php+: 

<php>
before_filter :set_expert_locale

public function set_expert_locale
  Ak::lang() = :expert if current_user.expert?
}
</php>
