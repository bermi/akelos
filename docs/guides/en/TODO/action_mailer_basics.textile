...
<shell>
./makelos generate mailer UserMailer
exists  app/models/
create  app/views/user_mailer
exists  test/unit/
create  test/fixtures/user_mailer
create  app/models/user_mailer.php
create  test/unit/user_mailer_test.php
</shell>
...
+app/models/user_mailer.php+ contains an empty mailer:

<php>
class UserMailer extends ActionMailer::Base
}
</php>
...
The keys of the array passed to +body+ become instance variables in the view. Thus, in our example the mailer view will have a +@user+ and a +@url+ instance variables available.

h5. Create a Mailer View

Create a file called +welcome_email.text.html.tpl+ in +app/views/user_mailer/+. This will be the template used for the email, formatted in HTML:
...
Had we wanted to send text-only emails, the file would have been called +welcome_email.text.plain.tpl+. Akelos sets the content type of the email to be the one in the filename.
...
<php>
Akelos::Initializer.run do |config|
  # ...
  config.active_record.observers = :user_observer
}
</php>

You can place the observer in +app/models+ where it will be loaded automatically by Akelos.

Now create a file called +user_observer.php+ in +app/models+ depending on where you stored it, and make it look like:

<php>
class UserObserver extends ActiveRecord::Observer
  public function after_create($user) {
...
So how does Action Mailer understand this +deliver_welcome_email+ call? If you read the documentation (http://api.akelos.org/files/vendor/akelos/actionmailer/README.html), you will find this in the "Sending Emails" section:
...
<php>
public function method_missing(method_symbol, *parameters)#:nodoc:
...
|body| The body of the email. This is either an array (in which case it specifies the variables to pass to the template when it is rendered), or a string, in which case it specifies the actual body of the message|
...
Mailer views are located in the +app/views/name_of_mailer_class+ directory. The specific mailer view is known to the class because it's name is the same as the mailer method. So for example, in our example from above, our mailer view for the +welcome_email+ method will be in +app/views/user_mailer/welcome_email.text.html.tpl+ for the HTML version and +welcome_email.text.plain.tpl+ for the plain text version.
...
class UserMailer extends ActionMailer::Base
  public function welcome_email($user) {
    ...
    # use some_other_template.text.(html|plain).tpl instead
...
Just like controller views, you can also have mailer layouts. The layout name needs to end in "_mailer" to be automatically recognized by your mailer as a layout. So in our UserMailer example, we need to call our layout +user_mailer.text.(html|plain).tpl+. In order to use a different file just use:

<php>
class UserMailer extends ActionMailer::Base
  layout 'awesome' # use awesome.text.(html|plain).tpl as the layout
}
</php>
...
the +ActionMailer::Base.defaultUrlOptions+ array as follows:

<tpl>
ActionMailer::Base.defaultUrlOptions['host'] = "example.com"
</tpl>

This can also be set as a configuration option in +config/environment.php+:

<tpl>
config.action_mailer.defaultUrlOptions = { 'host' => "example.com" }
</tpl>
...
Action Mailer will automatically send multipart emails if you have different templates for the same action. So, for our UserMailer example, if you have +welcome_email.text.plain.tpl+ and +welcome_email.text.html.tpl+ in +app/views/user_mailer+, Action Mailer will automatically send a multipart email with the HTML and text versions setup as different parts.
...
Receiving and parsing emails with Action Mailer can be a rather complex endeavour. Before your email reaches your Akelos app, you would have had to configure your system to somehow forward emails to your app, which needs to be listening for that. So, to receive emails in your Akelos app you'll need:
...
|helper(*args, &block)| Declare a helper: helper :foo requires 'foo_helper' and includes FooHelper in the template class.  helper FooHelper includes FooHelper in the template class.  helper { public function foo() "#{bar} is the very best" end } evaluates the block in the template class, adding method foo.  helper(:three, BlindHelper) { public function mice() 'mice' end } does all three. |
|helper_method| Declare a controller method as a helper.  For example, helper_method :link_to public function link_to(name, options) ... end makes the link_to controller method available in the view.|
...
The following configuration options are best made in one of the environment files (environment.php, production.php, etc...)

|template_root|Determines the base from which template references will be made.|
|logger|the logger is used for generating information on the mailing run if available. Can be set to nil for no logging. Compatible with both PHP's own Logger and Log4r loggers.|
...
Instructions copied from "this blog entry":http://www.fromjavatophp.com/2008/11/actionmailer-with-gmail-must-issue.html by Robert Zotter.
...
In +config/environment.php+, add the following line:
