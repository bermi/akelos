...
<php>
public function link_to_remote(name, options = {}, html_options = nil)	
</php>
...
* The second parameter, the +options+ array is the most interesting part as it has the AJAX specific stuff:
** *:url* This is the only parameter that is always required to generate the simplest remote link (technically speaking, it is not required, you can pass an empty +options+ array to +link_to_remote+ - but in this case the URL used for the POST request will be equal to your current URL which is probably not your intention). This URL points to your AJAX action handler. The URL is typically specified by Akelos REST view helpers, but you can use the +url_for+ format too.
...
The third parameter can either be a string, or an array of options to be passed to ActionView::Base#render - for example:
...
the DOM id of the element you wish to modify and a string or an array of options to be passed to ActionView::Base#render.
...
When using AJAX, you can't rely on the standard +$this->redirectTo(+ controller method - you have to use the +page+'s instance method, also called +$this->redirectTo(+:

<php>
page.$this->redirectTo( some_url
</php>
