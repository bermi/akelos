...
As an example, if a user goes to +/clients/new+ in your application to add a new client, Akelos will create an instance of +ClientsController+ and run the +new+ method. Note that the empty method from the example above could work just fine because Akelos will by default render the +new.html.tpl+ view unless the action says otherwise. The +new+ method could make available to the view a +@client+ instance variable by creating a new +Client+:
...
class ClientsController < ActionController::Base
  ...
  def create
    @client = Client.new($this->params['client'])
    if @client.save
      redirect_to @client
    else
...
The value of +$this->params['ids']+ will now be +["1", "2", "3"]+. Note that parameter values are always strings; Akelos makes no attempt to guess or cast the type.
...
When this form is submitted, the value of +$this->params['client']+ will be <tt>{"name" => 'Acme', "phone" => "12345", "address" => {"postcode" => "12345", "city" => "Carrot City"}}</tt>. Note the nested hash in +$this->params['client'][:address]+.
...
If you need a different session storage mechanism, you can change it in the +config/initializers/session_store.php+ file:

<php>
# Use the database for sessions instead of the cookie-based default,
# which shouldn't be used to store highly confidential information
# (create the session table with "./makelos db:sessions:create")
# ActionController::Base.session_store = :active_record_store
</php>

Akelos sets up a session key (the name of the cookie) and (for the CookieStore) a secret key used when signing the session data. These can also be changed in +config/initializers/session_store.php+:
...
class LoginsController < ApplicationController
  ...
  def create
    if user = User.authenticate($this->params['username'], $this->params['password'])
...
To remove something from the session, assign that key to be +null+:
...
Your application can store small amounts of data on the client -- called cookies -- that will be persisted across requests and even sessions. Akelos provides easy access to cookies via the +cookies+ method, which -- much like the +session+ -- works like a hash:
...
class CommentsController < ApplicationController
  ...
  def create
    @comment = Comment.new($this->params['comment'])
    if @comment.save
...
Note that while for session values you set the key to +null+, to delete a cookie value you should use +cookies.delete(:key)+.
...
class ApplicationController < ActionController::Base
  ...
  # Note that this is not common in PHP and is discouraged unless you
...
<php>
# Example taken from the Akelos API filter documentation:
# http://api.akelos.org/classes/ActionController/Filters/ClassMethods.html
class ApplicationController < ActionController::Base
...
class LoginsController < ApplicationController
  ...
  def create
    @user = User.authenticate($this->params['username'], $this->params['password'])
    if @user
...
Akelos adds this token to every form that's generated using the "form helpers":form_helpers.html, so most of the time you don't have to worry about it. If you're writing a form manually or need to add the token for another reason, it's available through the method +form_authenticity_token+:

The +form_authenticity_token+ generates a valid authentication token. That's useful in places where Akelos does not add it automatically, like in custom Ajax calls.
...
The request object contains a lot of useful information about the request coming in from the client. To get a full list of the available methods, refer to the "API documentation":http://api.akelos.org/classes/ActionController/AbstractRequest.html. Among the properties that you can access on this object are:
...
If you want to set custom headers for a response then +response.headers+ is the place to do it. The headers attribute is a hash which maps header names to their values, and Akelos will set some of them automatically. If you want to add or change a header, just assign it to +response.headers+ this way:
...
HTTP digest authentication is superior to the basic authentication as it does not require the client to send an unencrypted password over the network (though HTTP basic authentication is safe over HTTPS). Using digest authentication with Akelos is quite easy and only requires using one method, +authenticate_or_request_with_http_digest+.
...
As seen in the example above, the +authenticate_or_request_with_http_digest+ block takes only one argument - the username. And the block returns the password. Returning +false+ or +null+ from the +authenticate_or_request_with_http_digest+ will cause authentication failure.

h3. Streaming and File Downloads

Sometimes you may want to send a file to the user instead of rendering an HTML page. All controllers in Akelos have the +send_data+ and the +send_file+ methods, which will both stream data to the client. +send_file+ is a convenience method that lets you provide the name of a file on the disk and it will stream the contents of that file for you.
...
class ClientsController < ApplicationController
  ...
  def download_pdf
    client = Client.find($this->params['id'])
...
class ClientsController < ApplicationController
  ...
  def download_pdf
    client = Client.find($this->params['id'])
    send_data("#{Akelos.root}/files/clients/#{client.id}.pdf",
              :filename => "#{client.name}.pdf",
              :type => "application/pdf")
  end
end
...
While +send_data+ works just fine, if you are creating a RESTful application having separate actions for file downloads is usually not necessary. In REST terminology, the PDF file from the example above can be considered just another representation of the client resource. Akelos provides an easy and quite sleek way of doing "RESTful downloads". Here's how you can rewrite the example so that the PDF download is a part of the +show+ action, without any streaming:
...
In order for this example to work, you have to add the PDF MIME type to Akelos. This can be done by adding the following line to the file +config/initializers/mime_types.php+:
