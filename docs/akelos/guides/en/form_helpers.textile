...
IMPORTANT: Always use "GET" as the method for search forms. This allows users to bookmark a specific search and get back to it. More generally Akelos encourages you to use the right HTTP vtpl for an action.
...
This is a perfectly valid solution, but Akelos provides a less vtplose alternative: +options_from_collection_for_select+. This helper expects a collection of arbitrary objects and two additional arguments: the names of the methods to read the option *value* and *text* from, respectively:
...
Akelos provides the usual pair of helpers: the barebones +file_field_tag+ and the model oriented +file_field+. The only difference with other helpers is that you cannot set a default value for file inputs as this would have no meaning. As you would expect in the first case the uploaded file is in +params[:picture]+ and in the second case in +params[:person][:picture]+.
...
This would result in +params[:addresses]+ being an array of hashes with keys +line1+, +line2+ and +city+. Akelos decides to start accumulating values in a new hash whenever it encounters an input name that already exists in the current hash.
