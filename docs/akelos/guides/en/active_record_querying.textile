...
If you're used to using raw SQL to find database records then, generally, you will find that there are better ways to carry out the same operations in Akelos. Active Record insulates you from the need to use SQL in most cases.
...
TIP: For more information on the dangers of SQL injection, see the "Akelos Security Guide":security.html#sql-injection.
...
Just like in PHP. If you want a shorter syntax be sure to check out the "Hash Conditions":#hash-conditions section later on in the guide.
