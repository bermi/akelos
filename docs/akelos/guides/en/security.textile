...
If your web application is RESTful, you might be used to additional HTTP vtpls, such as PUT or DELETE. Most of today‘s web browsers, however do not support them - only GET and POST. Akelos uses a hidden +_method+ field to handle this barrier.

_(highlight)The verify method in a controller can make sure that specific actions may not be used over GET_. Here is an example to verify the use of the transfer action over POST. If the action comes in using any other vtpl, it redirects to the list action.
