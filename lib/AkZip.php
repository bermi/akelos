...
// ----- Constants
define('AK_ZIP_READ_BLOCK_SIZE', 2048);
// ----- File list separator
define('AK_ZIP_SEPARATOR', ',');
...
//       Samples :
// define( 'AK_ZIP_TEMPORARY_DIR', '/temp/' );
// define( 'AK_ZIP_TEMPORARY_DIR', 'C:/Temp/' );
define('AK_ZIP_TEMPORARY_DIR', '');
// ----- Error codes
define('AK_ZIP_ERR_NO_ERROR', 0);
define('AK_ZIP_ERR_WRITE_OPEN_FAIL', -1);
define('AK_ZIP_ERR_READ_OPEN_FAIL', -2);
define('AK_ZIP_ERR_INVALID_PARAMETER', -3);
define('AK_ZIP_ERR_MISSING_FILE', -4);
define('AK_ZIP_ERR_FILENAME_TOO_LONG', -5);
define('AK_ZIP_ERR_INVALID_ZIP', -6);
define('AK_ZIP_ERR_BAD_EXTRACTED_FILE', -7);
define('AK_ZIP_ERR_DIR_CREATE_FAIL', -8);
define('AK_ZIP_ERR_BAD_EXTENSION', -9);
define('AK_ZIP_ERR_BAD_FORMAT', -10);
define('AK_ZIP_ERR_DELETE_FILE_FAIL', -11);
define('AK_ZIP_ERR_RENAME_FILE_FAIL', -12);
define('AK_ZIP_ERR_BAD_CHECKSUM', -13);
define('AK_ZIP_ERR_INVALID_AK_ZIP', -14);
define('AK_ZIP_ERR_MISSING_OPTION_VALUE', -15);
define('AK_ZIP_ERR_INVALID_PARAM_VALUE', -16);
// ----- Warning codes
define('AK_ZIP_WARN_NO_WARNING', 0);
define('AK_ZIP_WARN_FILE_EXIST', 1);
// ----- Methods parameters
define('AK_ZIP_PARAM_PATH', 'path');
define('AK_ZIP_PARAM_ADD_PATH', 'add_path');
define('AK_ZIP_PARAM_REMOVE_PATH', 'remove_path');
define('AK_ZIP_PARAM_REMOVE_ALL_PATH', 'remove_all_path');
define('AK_ZIP_PARAM_SET_CHMOD', 'set_chmod');
define('AK_ZIP_PARAM_EXTRACT_AS_STRING', 'extract_as_string');
...
define('AK_ZIP_PARAM_PRE_ADD', 'callback_pre_add');
define('AK_ZIP_PARAM_POST_ADD', 'callback_post_add');
/**
...
class AkZip
{
    ...
    function create($p_filelist, $p_params = 0) 
    {
        ...
        // ----- Look if the $p_filelist is a string
            ...
            $v_list = explode(AK_ZIP_SEPARATOR, $p_filelist);
            $v_result = $this->_create($v_list, $p_result_list, $p_params);
        }
        // ----- Invalid variable
        else {
            $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, 'Invalid variable type p_filelist');
            $v_result = AK_ZIP_ERR_INVALID_PARAMETER;
        }
        if ($v_result != 1) {
            return 0;
        }
    ...
    function add($p_filelist, $p_params = 0) 
    {
        ...
        // ----- Invalid variable
            ...
            $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, "add() : Invalid variable type p_filelist");
            $v_result = AK_ZIP_ERR_INVALID_PARAMETER;
        }
        if ($v_result != 1) {
            return 0;
    ...
    function properties() 
    {
        ...
        if (@is_file($this->_zipname)) {
            ...
            if (($this->_zip_fd = @fopen($this->_zipname, 'rb')) == 0) {
                $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, 'Unable to open archive \''.$this->_zipname.'\' in binary read mode');
                return 0;
            }
    ...
    function duplicate($p_archive) 
    {
        ...
        // ----- Look if the $p_archive is a string (so a filename)
            ...
            if (!is_file($p_archive)) {
                $this->_errorLog(AK_ZIP_ERR_MISSING_FILE, "No file with filename '".$p_archive."'");
                $v_result = AK_ZIP_ERR_MISSING_FILE;
            } else {
        ...
        // ----- Invalid variable
            ...
            $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, "Invalid variable type p_archive_to_add");
            $v_result = AK_ZIP_ERR_INVALID_PARAMETER;
        }
        return $v_result;
    }
    ...
    function merge($p_archive_to_add) 
    {
        ...
        // ----- Look if the $p_archive_to_add is a AkZip object
        ...
        // ----- Look if the $p_archive_to_add is a string (so a filename)
            ...
            $v_object_archive = new AkZip($p_archive_to_add);
            // ----- Merge the archive
    ...
    function errorName($p_with_code = false) 
    {
        ...
        for (reset($v_const_list) ; list($v_key, $v_value) = each($v_const_list) ;) {
            if (substr($v_key, 0, strlen('AK_ZIP_ERR_')) == 'AK_ZIP_ERR_') {
    ...
    function _checkFormat($p_level = 0) 
    {
        ...
        if (!is_file($this->_zipname)) {
            // ----- Error log
            $this->_errorLog(AK_ZIP_ERR_MISSING_FILE, "Missing archive file '".$this->_zipname."'");
            return (false);
        }
        // ----- Check that the file is readeable
        if (!is_readable($this->_zipname)) {
            // ----- Error log
            $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, "Unable to read archive '".$this->_zipname."'");
            return (false);
        }
    ...
    function _add($p_list, &$p_result_list, &$p_params) 
    {
        ...
        if (($v_zip_temp_fd = @fopen($v_zip_temp_name, 'wb')) == 0) {
            $this->_closeFd();
            $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, 'Unable to open temporary file \''.$v_zip_temp_name.'\' in binary write mode');
        ...
        while ($v_size != 0) {
            $v_read_size = ($v_size < AK_ZIP_READ_BLOCK_SIZE ? $v_size : AK_ZIP_READ_BLOCK_SIZE);
    ...
    function _openFd($p_mode) 
    {
        ...
        if ($this->_zip_fd != 0) {
            $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, 'Zip file \''.$this->_zipname.'\' already open');
            return AkZip::errorCode();
        }
        // ----- Open the zip file
        if (($this->_zip_fd = @fopen($this->_zipname, $p_mode)) == 0) {
            $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, 'Unable to open archive \''.$this->_zipname.'\' in '.$p_mode.' mode');
            return AkZip::errorCode();
        }
        // ----- Return
        return $v_result;
    ...
    function _addFileList($p_list, &$p_result_list, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_params) 
    {
        ...
        for ($j = 0 ; ($j < count($p_list)) && ($v_result == 1) ; $j++) {
            ...
            if (!file_exists($p_filename)) {
                $this->_errorLog(AK_ZIP_ERR_MISSING_FILE, "File '$p_filename' does not exists");
                return AkZip::errorCode();
            }
    ...
    function _addFile($p_filename, &$p_header, $p_add_dir, $p_remove_dir, $p_remove_all_dir, &$p_params) 
    {
        ...
        if ($p_filename == "") {
            // ----- Error log
            $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, "Invalid file list parameter (invalid or empty list)");
            // ----- Return
            return AkZip::errorCode();
        }
        ...
        */
        ...
        if ((isset($p_params[AK_ZIP_PARAM_PRE_ADD])) && ($p_params[AK_ZIP_PARAM_PRE_ADD] != '')) {
            ...
            // header.
            eval('$v_result = '.$p_params[AK_ZIP_PARAM_PRE_ADD].'(AK_ZIP_PARAM_PRE_ADD, $v_local_header);');
            if ($v_result == 0) {
        ...
        if ($p_header['status'] == 'ok') {
            ...
            if (is_file($p_filename)) {
                ...
                if (($v_file = @fopen($p_filename, "rb")) == 0) {
                    $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, "Unable to open file '$p_filename' in binary read mode");
                    return AkZip::errorCode();
                }
        ...
        // ----- Look for pre-add callback
        if ((isset($p_params[AK_ZIP_PARAM_POST_ADD])) && ($p_params[AK_ZIP_PARAM_POST_ADD] != '')) {
            // ----- Generate a local information
            $v_local_header = array();
            $this->_convertHeader2FileInfo($p_header, $v_local_header);
            // ----- Call the callback
            // Here I do not use call_user_func() because I need to send a reference to the
            // header.
            eval('$v_result = '.$p_params[AK_ZIP_PARAM_POST_ADD].'(AK_ZIP_PARAM_POST_ADD, $v_local_header);');
            if ($v_result == 0) {
                // ----- Ignored
                $v_result = 1;
            }
    ...
    function _extractByRule(&$p_file_list, &$p_params) 
    {
        ...
        for ($i = 0, $v_nb_extracted = 0 ; $i < $v_central_dir['entries'] ; $i++) {
            ...
            if ((isset($p_params[AK_ZIP_PARAM_BY_NAME])) && ($p_params[AK_ZIP_PARAM_BY_NAME] != 0)) {
                ...
                for ($j = 0 ; ($j < sizeof($p_params[AK_ZIP_PARAM_BY_NAME])) && (!$v_extract) ; $j++) {
                    // ----- Look for a directory
                    if (substr($p_params[AK_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
                        // ----- Look if the directory is in the filename path
                        if ((strlen($v_header['stored_filename']) > strlen($p_params[AK_ZIP_PARAM_BY_NAME][$j])) && (substr($v_header['stored_filename'], 0, strlen($p_params[AK_ZIP_PARAM_BY_NAME][$j])) == $p_params[AK_ZIP_PARAM_BY_NAME][$j])) {
                            $v_extract = true;
                        }
                    }
                    // ----- Look for a filename
            ...
            else if ((isset($p_params[AK_ZIP_PARAM_BY_PREG])) && ($p_params[AK_ZIP_PARAM_BY_PREG] != "")) {
                if (preg_match($p_params[AK_ZIP_PARAM_BY_PREG], $v_header['stored_filename'])) {
                    $v_extract = true;
                }
            }
            // ----- Look for extract by index rule
            else if ((isset($p_params[AK_ZIP_PARAM_BY_INDEX])) && ($p_params[AK_ZIP_PARAM_BY_INDEX] != 0)) {
                // ----- Look if the index is in the list
                for ($j = $j_start ; ($j < sizeof($p_params[AK_ZIP_PARAM_BY_INDEX])) && (!$v_extract) ; $j++) {
            ...
            if ($v_extract) {
                ...
                if ($p_params[AK_ZIP_PARAM_EXTRACT_AS_STRING]) {
                    // ----- Extracting the file
    ...
    function _extractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_params) 
    {
        ...
        if ((isset($p_params[AK_ZIP_PARAM_PRE_EXTRACT])) && ($p_params[AK_ZIP_PARAM_PRE_EXTRACT] != '')) {
            // ----- Generate a local information
            $v_local_header = array();
            ...
            // Here I do not use call_user_func() because I need to send a reference to the
            // header.
            eval('$v_result = '.$p_params[AK_ZIP_PARAM_PRE_EXTRACT].'(AK_ZIP_PARAM_PRE_EXTRACT, $v_local_header);');
            if ($v_result == 0) {
                // ----- Change the file status
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (!(($p_entry['external']&0x00000010) == 0x00000010)) {
                ...
                if ($p_entry['compressed_size'] == $p_entry['size']) {
                    ...
                    // ----- Read the file by AK_ZIP_READ_BLOCK_SIZE octets blocks
                ...
                if ((isset($p_params[AK_ZIP_PARAM_SET_CHMOD])) && ($p_params[AK_ZIP_PARAM_SET_CHMOD] != 0)) {
                    // ----- Change the mode of the file
                    chmod($p_entry['filename'], $p_params[AK_ZIP_PARAM_SET_CHMOD]);
                }
            }
        }
        // ----- Look for post-extract callback
        if ((isset($p_params[AK_ZIP_PARAM_POST_EXTRACT])) && ($p_params[AK_ZIP_PARAM_POST_EXTRACT] != '')) {
            // ----- Generate a local information
            $v_local_header = array();
            $this->_convertHeader2FileInfo($p_entry, $v_local_header);
            // ----- Call the callback
            // Here I do not use call_user_func() because I need to send a reference to the
            // header.
            eval('$v_result = '.$p_params[AK_ZIP_PARAM_POST_EXTRACT].'(AK_ZIP_PARAM_POST_EXTRACT, $v_local_header);');
        }
        // ----- Return
        return $v_result;
    }
    ...
    function _readFileHeader(&$p_header) 
    {
        ...
        if ($v_data['id'] != 0x04034b50) {
            // ----- Error log
            $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, 'Invalid archive structure');
            // ----- Return
            return AkZip::errorCode();
        }
        ...
        if (strlen($v_binary_data) != 26) {
            ...
            // ----- Error log
            $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, "Invalid block size : ".strlen($v_binary_data));
            // ----- Return
            return AkZip::errorCode();
        }
        // ----- Extract the values
    ...
    function _readCentralFileHeader(&$p_header) 
    {
        ...
        // ----- Get filename
        if ($p_header['filename_len'] != 0) $p_header['filename'] = fread($this->_zip_fd, $p_header['filename_len']);
        else $p_header['filename'] = '';
        // ----- Get extra
        if ($p_header['extra_len'] != 0) $p_header['extra'] = fread($this->_zip_fd, $p_header['extra_len']);
        else $p_header['extra'] = '';
        // ----- Get comment
        if ($p_header['comment_len'] != 0) $p_header['comment'] = fread($this->_zip_fd, $p_header['comment_len']);
    ...
    function _readEndCentralDir(&$p_central_dir) 
    {
        ...
        if (@ftell($this->_zip_fd) != $v_size) {
            $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, 'Unable to go to the end of the archive \''.$this->_zipname.'\'');
            return AkZip::errorCode();
        }
        ...
        if ($v_size > 26) {
            ...
            if (($v_pos = @ftell($this->_zip_fd)) != ($v_size-22)) {
                $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, 'Unable to seek back to the middle of the archive \''.$this->_zipname.'\'');
                return AkZip::errorCode();
            }
            // ----- Read for bytes
            $v_binary_data = @fread($this->_zip_fd, 4);
        ...
        if (!$v_found) {
            ...
            if ($v_pos == $v_size) {
                $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, "Unable to find End of Central Dir Record signature");
                return AkZip::errorCode();
            }
        }
        ...
        if (strlen($v_binary_data) != 18) {
            $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, "Invalid End of Central Dir Record size : ".strlen($v_binary_data));
            return AkZip::errorCode();
        }
        // ----- Extract the values
        ...
        if (($v_pos+$v_data['comment_size']+18) != $v_size) {
            $this->_errorLog(AK_ZIP_ERR_BAD_FORMAT, "Fail to find the right signature");
            return AkZip::errorCode();
        }
        // ----- Get comment
        if ($v_data['comment_size'] != 0) $p_central_dir['comment'] = fread($this->_zip_fd, $v_data['comment_size']);
                ...
                for ($j = 0 ; ($j < sizeof($p_params[AK_ZIP_PARAM_BY_NAME])) && (!$v_found) ; $j++) {
                    ...
                    if (substr($p_params[AK_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
                        ...
                        if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen($p_params[AK_ZIP_PARAM_BY_NAME][$j])) && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, strlen($p_params[AK_ZIP_PARAM_BY_NAME][$j])) == $p_params[AK_ZIP_PARAM_BY_NAME][$j])) {
                            $v_found = true;
                        } elseif ((($v_header_list[$v_nb_extracted]['external']&0x00000010) == 0x00000010) /* Indicates a folder */ && ($v_header_list[$v_nb_extracted]['stored_filename'].'/' == $p_params[AK_ZIP_PARAM_BY_NAME][$j])) {
                            $v_found = true;
                        }
                    }
                    // ----- Look for a filename
    ...
    function _dirCheck($p_dir, $p_is_dir = false) 
    {
        ...
        if (!@mkdir($p_dir, 0777)) {
            $this->_errorLog(AK_ZIP_ERR_DIR_CREATE_FAIL, "Unable to create directory '$p_dir'");
            return AkZip::errorCode();
        }
        // ----- Return
        return $v_result;
    ...
    function _duplicate($p_archive_filename) 
    {
        ...
        if (($v_zip_temp_fd = @fopen($p_archive_filename, 'rb')) == 0) {
            $this->_closeFd();
            $this->_errorLog(AK_ZIP_ERR_READ_OPEN_FAIL, 'Unable to open archive file \''.$p_archive_filename.'\' in binary write mode');
            return AkZip::errorCode();
        }
        // ----- Copy the files from the archive to the temporary file
        // TBC : Here I should better append the file and go back to erase the
    ...
    function _check_parameters(&$p_params, $p_default) 
    {
        ...
        if (!is_array($p_params)) {
            $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, 'Unsupported parameter, waiting for an array');
            return AkZip::errorCode();
        }
        // ----- Check that all the params are valid
        for (reset($p_params) ; list($v_key, $v_value) = each($p_params) ;) {
            if (!isset($p_default[$v_key])) {
                $this->_errorLog(AK_ZIP_ERR_INVALID_PARAMETER, 'Unsupported parameter with key \''.$v_key.'\'');
                return AkZip::errorCode();
            }
        }
        ...
        for ($i = 0 ; $i < sizeof($v_callback_list) ; $i++) {
            ...
            if ((isset($p_params[$v_key])) && ($p_params[$v_key] != '')) {
                if (!function_exists($p_params[$v_key])) {
                    $this->_errorLog(AK_ZIP_ERR_INVALID_PARAM_VALUE, "Callback '".$p_params[$v_key]."()' is not an existing function for "."parameter '".$v_key."'");
                    return AkZip::errorCode();
                }
            }
        }
    ...
    function _tool_CopyBlock($p_src, $p_dest, $p_size, $p_mode = 0) 
    {
        ...
        if ($p_mode == 0) {
            while ($p_size != 0) {
                $v_read_size = ($p_size < AK_ZIP_READ_BLOCK_SIZE ? $p_size : AK_ZIP_READ_BLOCK_SIZE);
