...
class CI_Tests
{
    ...
    public $settings = array('executables'=>array(),'environments'=>array(),'default_executables'=>array());
    public $configured = array('ci-config.yaml'=>false,'environments'=>false);
    public $target_files;
    public $test_files;
    public $target_executables;
    public $target_environments;
    public $debug = false;
    public $report_environments = array();
    ...
    private function _promptForDbConfig($env)
    {
    ...
    private function _createDbConfig($env)
    {
    ...
    private function _checkTestInstallation()
    {
        ...
        if (version_compare($version,0,'>')) {
            ...
            $install_akelos = preg_match('/^y/i', $this->promptUserVar('Could not find an akelos application to test against at: '.$this->settings['test-installation']."\nWhould you like to create it now?",  array('default'=>'yes')));
            if(!$install_akelos){
    ...
    private function _checkWebserver2($url)
    {
        $return = true;
        $testIndexPage = @file_get_contents($url.'/');
        
        if (!strstr($testIndexPage, 'Test::page::index')) {
    ...
    private function _checkWebServer($url, $test_installation = null)
    {
        $return = true;
    ...
    private function _checkDbConfig($type, $settings = array())
    {
    ...
    private function _checkExecutables($executables = array())
    {
    ...
    private function _createCiPhpConfigFile()
    {
    ...
    private function _createCiConfigFile()
    {
    ...
    private function _promptForPhp($php)
    {
    ...
    private function _promptForTestingUrl($test_installation)
    {
    ...
    private function _createTestInstallation($testDir = null)
    {
        if ($testDir == null) {
    ...
    private function _fixTestInstallationPermissions()
    {
    ...
    private function _setupMemcache()
    {
    ...
    private function _createMemcachedConfig($socket)
    {
    ...
    private function _checkMemcacheInstallation($socket = null)
    {
        if ($socket == null) {
    ...
    private function _configureMemcache()
    {
        require_once(AK_BASE_DIR.DS.'lib'.DS.'AkCache'.DS.'AkMemcache.php');
        $memcache = new AkMemcache();
    ...
    public function loadSettings($filename=AK_CI_CONFIG_FILE)
    {
    ...
    public function constructTestFilename($filename)
    {
    ...
    public function filesToRun()
    {
        return $this->target_files;
    }

    public function executablesToRun()
    {
        return $this->target_executables;
    }

    public function environmentsToRun()
    {
        return $this->target_environments;
    }

    public function timesToRun()
    {
        return $this->options['repeat'];
    }

    public function isValidCombination($environment,$php_version)
    {
    ...
    public function prepareEnvironment($environment)
    {
    ...
    private function _generateSummary()
    {
    ...
    private function _generateReport($xmlFile,$php,$env)
    {
    ...
    public function runCommand($php,$filename,$environment)
    {
    ...
    public function drawBox($message)
    {
        $this->drawNewline();
        $this->drawLine();
        $this->drawNewline();
    ...
    public function drawRepeatIndicator($actual)
    {
    ...
    public function drawLine($char='-',$num=80)
    {
        echo str_pad('',$num,$char);
    }

    public function drawNewline($multiplier=1)
    {
        echo str_repeat("\n\r",$multiplier);
    }
